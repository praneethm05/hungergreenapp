...
const MobileSignin = ({ navigation }: { navigation: any }) => {
  const { signIn, isLoaded } = useSignIn();
  const COUNTRY_CODE = "+91"; // Hardcoded India country code
  ...
  const handleSendOTP = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://192.168.1.6:5500/users/?phone=${encodeURIComponent(fullPhone)}`);
      if (!response.ok) {
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (result.status === "complete") {
        // Set the active session so Clerk updates the auth state
        // await signIn.setActive({ session: result.createdSessionId });
        Alert.alert("Yaaay!", "You are now logged in!");
      } else {
